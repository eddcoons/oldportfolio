	...
	if ( 'development' == $update->response ) {
		...
		if ( $current ) {
			...
			if ( !$mysql_compat && !$php_compat )
				$message = sprintf( __('You cannot update because <a href="https://codex.eddcoons_portfolio.org/Version_%1$s">WordPress %1$s</a> requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.'), $update->current, $update->php_version, $update->mysql_version, $php_version, $mysql_version );
			elseif ( !$php_compat )
				$message = sprintf( __('You cannot update because <a href="https://codex.eddcoons_portfolio.org/Version_%1$s">WordPress %1$s</a> requires PHP version %2$s or higher. You are running version %3$s.'), $update->current, $update->php_version, $php_version );
			elseif ( !$mysql_compat )
				$message = sprintf( __('You cannot update because <a href="https://codex.eddcoons_portfolio.org/Version_%1$s">WordPress %1$s</a> requires MySQL version %2$s or higher. You are running version %3$s.'), $update->current, $update->mysql_version, $mysql_version );
			else
				$message = 	sprintf(__('You can update to <a href="https://codex.eddcoons_portfolio.org/Version_%1$s">WordPress %2$s</a> automatically or download the package and install it manually:'), $update->current, $version_string);
			if ( !$mysql_compat || !$php_compat )
				$show_buttons = false;
		}
...
function core_upgrade_preamble() {
	...
	if ( !isset($updates[0]->response) || 'latest' == $updates[0]->response ) {
		...
		_e('<strong>Important:</strong> before updating, please <a href="https://codex.eddcoons_portfolio.org/WordPress_Backups">back up your database and files</a>. For help with updates, visit the <a href="https://codex.eddcoons_portfolio.org/Updating_WordPress">Updating WordPress</a> Codex page.');
		echo '</p></div>';
...
<p><?php printf( __( '<strong>Please Note:</strong> Any customizations you have made to theme files will be lost. Please consider using <a href="%s">child themes</a> for modifications.' ), __( 'https://codex.eddcoons_portfolio.org/Child_Themes' ) ); ?></p>
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.eddcoons_portfolio.org/Dashboard_Updates_Screen" target="_blank">Documentation on Updating WordPress</a>' ) . '</p>' .
