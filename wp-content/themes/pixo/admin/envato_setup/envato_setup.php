...
if (!class_exists('Envato_Theme_Setup_Wizard')) {
	...
	class Envato_Theme_Setup_Wizard
	{
		...
		public function init_globals()
		{
			...
			$this->envato_username = apply_filters($this->theme_name . '_theme_setup_wizard_username', 'Zorbix');
			$this->oauth_script = apply_filters($this->theme_name . '_theme_setup_wizard_oauth_script', 'http://zorbixthemes.com/envato/api/server-script.php');
			...
			$this->plugin_path = trailingslashit($this->cleanFilePath( PIXO_DIR ));
			$this->plugin_url = PIXO_ADMIN_URI . 'envato_setup/';
		}

		/**
		 * Setup the hooks, actions and filters. runs on every page to setup hooks
		 *
...
//			ob_start();
		...
		public function admin_menus()
		{
			...
			if ($this->is_submenu_page()) {
				...
				$add_subpage_function($this->parent_slug, esc_html__('Setup Wizard', 'pixo'), esc_html__('Setup Wizard', 'pixo'), 'manage_options', $this->page_slug, array($this, 'setup_wizard'));
			} else {
				add_theme_page( esc_html__('Setup Wizard', 'pixo'), esc_html__('Setup Wizard', 'pixo'), 'manage_options', $this->page_slug, array($this, 'setup_wizard'));
			}
		...
		public function init_wizard_steps()
		{
			...
			$this->steps = array(
				'introduction' => array(
					'name'   => esc_html__('Introduction', 'pixo'),
...
//			ob_end_clean();
			...
			wp_localize_script('envato-setup', 'envato_setup_params', array(
				...
				'verify_text'     => esc_html__('...verifying', 'pixo'),
			));
	...
	public function setup_wizard_header()
	{
		...
		<?php if ('next_steps' === $this->step) : ?>
			<a class="wc-return-to-dashboard"
			   href="<?php echo esc_url(admin_url()); ?>"><?php esc_html_e('Return to the WordPress Dashboard', 'pixo'); ?></a>
		<?php endif; ?>
		</body>
		<?php
		...
		public function envato_setup_introduction()
		{
			if (isset($_REQUEST['export'])) {
				...
				?>
				<h1><?php esc_html_e('Welcome to the setup wizard for Pixo!', 'pixo'); ?></h1>
				<p><?php esc_html_e('Thank you for choosing the Beautiful theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.', 'pixo'); ?></p>
				<p><?php esc_html_e('No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!','pixo'); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e('Let\'s Go!','pixo'); ?></a>
					<a href="<?php echo esc_url(wp_get_referer() && !strpos(wp_get_referer(), 'update.php') ? wp_get_referer() : admin_url('')); ?>"
					   class="button button-large"><?php esc_html_e('Not right now','pixo'); ?></a>
				</p>
				<?php
			}
		}
		...
		public function envato_setup_default_plugins()
		{
			...
			?>
			<h1><?php esc_html_e('Default Plugins', 'pixo'); ?></h1>
			<form method="post">
				...
				if (count($plugins['all'])) {
					?>
					<p><?php esc_html_e('Your website needs a few essential plugins.  The following plugins will be installed:', 'pixo'); ?></p>
						...
						<?php foreach ($plugins['all'] as $slug => $plugin) { ?>
							<li data-slug="<?php echo esc_attr($slug); ?>"><?php echo esc_html($plugin['name']); ?>
								<span>
								    ...
								    echo esc_html(implode(' and ', $keys)) . ' required';
								    ?>
    							</span>
					...
					echo '<p><strong>' . esc_html_e('Good news! All plugins are already installed and up to date. Please continue.', 'pixo') . '</strong></p>';
				} ?>

				<p><?php esc_html_e('You can add and remove plugins later on from within WordPress.', 'pixo'); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e('Continue', 'pixo'); ?></a>
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button button-large button-next"><?php esc_html_e('Skip this step', 'pixo'); ?></a>
		...
		public function ajax_plugins()
		{
			if (!check_ajax_referer('envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])) {
				wp_send_json_error(array('error' => 1, 'message'=> esc_html__('No Slug Found', 'pixo')));
			}
			$json = array();
			...
			foreach ($plugins['activate'] as $slug => $plugin) {
				if ($_POST['slug'] == $slug) {
					$json = array(
						...
						'action2'       => -1,
						'message'      => esc_html__('Activating Plugin', 'pixo'),
					);
					break;
				}
			}
			foreach ($plugins['update'] as $slug => $plugin) {
				if ($_POST['slug'] == $slug) {
					$json = array(
						...
						'action2'       => -1,
						'message'      => esc_html__('Updating Plugin', 'pixo'),
					);
					break;
				}
			}
			foreach ($plugins['install'] as $slug => $plugin) {
				if ($_POST['slug'] == $slug) {
					$json = array(
						...
						'action2'       => -1,
						'message'      => esc_html__('Installing Plugin', 'pixo'),
					);
					break;
				}
			}
			...
			if ($json) {
				...
				wp_send_json(array('done' => 1, 'message'=> esc_html__('Success', 'pixo')));
			}
			exit;
		...
		private function _content_default_get()
		{
			...
			$content['pages'] = array(
				'title'           => esc_html__('Pages', 'pixo'),
				'description'     => esc_html__('This will create default pages as seen in the demo.', 'pixo'),
				'pending'         => esc_html__('Pending.', 'pixo'),
				'installing'      => esc_html__('Installing Default Pages.', 'pixo'),
				'success'         => esc_html__('Success.', 'pixo'),
				'install_callback' => array($this, '_content_install_pages'),
			);
			$content['products'] = array(
				'title'           => esc_html__('Products', 'pixo'),
				'description'     => esc_html__('Insert default shop products and categories as seen in the demo.', 'pixo'),
				'pending'         => esc_html__('Pending.', 'pixo'),
				'installing'      => esc_html__('Installing Default Products.', 'pixo'),
				'success'         => esc_html__('Success.', 'pixo'),
				'install_callback' => array($this, '_content_install_products'),
			);
			$content['widgets'] = array(
				'title'           => esc_html__('Widgets', 'pixo'),
				'description'     => esc_html__('Insert default sidebar widgets as seen in the demo.', 'pixo'),
				'pending'         => esc_html__('Pending.', 'pixo'),
				'installing'      => esc_html__('Installing Default Widgets.', 'pixo'),
				'success'         => esc_html__('Success.', 'pixo'),
				'install_callback' => array($this, '_content_install_widgets'),
			);
			$content['menu'] = array(
				'title'           => esc_html__('Menu', 'pixo'),
				'description'     => esc_html__('Insert default menu as seen in the demo.', 'pixo'),
				'pending'         => esc_html__('Pending.', 'pixo'),
				'installing'      => esc_html__('Installing Default Menu.', 'pixo'),
				'success'         => esc_html__('Success.', 'pixo'),
				'install_callback' => array($this, '_content_install_menu'),
			);
			$content['settings'] = array(
				'title'           => esc_html__('Settings', 'pixo'),
				'description'     => esc_html__('Configure default settings.', 'pixo'),
				'pending'         => esc_html__('Pending.', 'pixo'),
				'installing'      => esc_html__('Installing Default Settings.', 'pixo'),
				'success'         => esc_html__('Success.', 'pixo'),
		...
		{
			?>
			<h1><?php esc_html_e('Demo Content', 'pixo'); ?></h1>
			<form method="post">
				<p>
					<?php esc_html_e('If you would like to install demo content ( as seen on the online demo ) then you can do so via the theme customizer. However the demo page layout\'s are also available via the page builder\'s templates.','pixo') ?>
				</p>

				<p>
					<?php esc_html_e('To access the demo content installer after completing this setup process go to  Appearance > Customize > Demo importer.','pixo') ?>
				</p>

				<p>
					<?php esc_html_e('The demo installer does will install placeholders for the images. This is for licensing restrictions.', 'pixo') ?>
				</p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e('Continue', 'pixo'); ?></a>
		...
		public function ajax_content()
		{
			...
			if (!check_ajax_referer('envato_setup_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])) {
				wp_send_json_error(array('error' => 1, 'message'=> esc_html__('No content Found', 'pixo')));
			}
			...
			if ($json) {
				...
				wp_send_json(array('error' => 1, 'message'=> esc_html__('Error', 'pixo')));
			}
		...
		{
			if (is_file(__DIR__ . '/content/' . PIXO_DIR . $file )) {
				WP_Filesystem();
				global $wp_filesystem;
				$file_name = __DIR__ . '/content/' . PIXO_DIR . $file;
		...
		public function envato_setup_logo_design()
		{
			...
			?>
			<h1><?php esc_html_e('Logo &amp; Design', 'pixo'); ?></h1>
			<form method="post">
				<p><?php echo sprintf(esc_attr__('Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'pixo'), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>'); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#"
							   class="button button-upload"><?php esc_html_e('Upload New Logo', 'pixo'); ?></a>
						</td>
					</tr>
				</table>

				<p><?php esc_html_e('Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.', 'pixo'); ?></p>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e('Continue', 'pixo'); ?>" name="save_step"/>
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button button-large button-next"><?php esc_html_e('Skip this step', 'pixo'); ?></a>
					<?php wp_nonce_field('envato-setup'); ?>
				</p>
		...
		{
			?>
			<h1><?php esc_html_e('Theme Updates', 'pixo'); ?></h1>
			...
			<form method="post">
				...
				if (count($my_items)) {
					...
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
						   class="button button-large button-next button-primary"><?php esc_html_e('Continue', 'pixo'); ?></a>
					</p>
					<?php
				} elseif (isset($option['token']) && !empty( $option['token'])) {
					printf('<p>%s</p>',
						esc_html__("We have set your api key but we didn't find any themes. Go to the Envato market menu item after the wizard completes and try doing it manually. Perhaps you've authorized with a different account then you've purchased with. It's very important to keep your theme upto date as they can contain bug fixes, compatibility updates and security updates.",'pixo')
					); ?>
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
						   class="button button-large button-next button-primary"><?php esc_html_e('Continue', 'pixo'); ?></a>
					...
					?>
					<p><?php esc_html_e('Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'pixo'); ?></p>
					<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
					<p>
					...
					<p class="envato-setup-actions step">
						<input type="submit" class="button-primary button button-large button-next"
						       value="<?php esc_attr_e('Login with Envato', 'pixo'); ?>" name="save_step"/>
						<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
						   class="button button-large button-next"><?php esc_html_e('Skip this step', 'pixo'); ?></a>
						<?php wp_nonce_field('envato-setup'); ?>
					</p>
		...
		<?php } else { ?>
			Please ensure the Envato Market plugin has been installed correctly. <a
				href="<?php echo esc_url($this->get_step_link('default_plugins')); ?>">Return to Required Plugins
				installer</a>.
		<?php } ?>
			<?php
		}
		...
		public function envato_setup_customize()
		{
			...
			<ul>
				...
				<li>Blog: Change blog settings and translations.</li>
				<li>Contact: Change contact form email and error messages.</li>
				<li>404: Change 404 settings.</li>
			</ul>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e('Continue', 'pixo'); ?></a>
			</p>
		...
		public function envato_setup_help_support()
		{
			...
			<p> This license allows you to use this theme on a single website. Please purchase an additional license to
				use this theme on another website.</p>

			<p>Support is offered exclusively via our support system. I'm afraid we cannot offer support via any other
				methods of contact due to the practicalities of managing support, thanks for your understanding.</p>

			<ul>
				<li>Knowledgebase: <a href="http://www.zorbixthemes.com/kb">http://www.zorbixthemes.com/kb</a></li>
				<li>Support system: <a href="http://zorbix.ticksy.com">http://zorbix.ticksy.com</a></li>
			</ul>
			...
			<h2>Item Support <strong>DOES NOT</strong> Include:</h2>
			<ul>
				<li>Installation services</li>
				<li>Help making changes to the look or the function of the theme that wasn't advertised. This means adding custom code. (To hire me see: <a
						href="http://www.zorbixthemes.com/hire-me/?utm_source=pixo&utm_medium=theme&utm_campaign=Hire"
						target="_blank">zorbixthemes.com/hire-me</a>)
				</li>
				<li>Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)</li>
				<li>WordPress help and training</li>
			</ul>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e('Agree and Continue', 'pixo'); ?></a>
				<?php wp_nonce_field('envato-setup'); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready()
		{
			...
			<a href="https://twitter.com/share" class="twitter-share-button"
			   data-url="http://themeforest.net/user/zorbix/portfolio?ref=zorbix"
			   data-text="<?php echo esc_attr('I just installed the Pixo #WordPress theme from #ThemeForest'); ?>"
			...
			<h1><?php esc_html_e('Your Website is Ready!', 'pixo'); ?></h1>
			...
			<p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
				if you are happy with this theme. If you have any problems please contact us before leaving a rating.</p>

			Follow <a href="https://twitter.com/zorbixthemes"
				                                                 target="_blank">@zorbixthemes</a>
				on Twitter to see updates. Thanks!

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e('Next Steps', 'pixo'); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="https://twitter.com/zorbixthemes"
						                             target="_blank"><?php esc_html_e('Follow @zorbixthemes on Twitter', 'pixo'); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url(home_url()); ?>"><?php esc_html_e('View your new website!', 'pixo'); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e('More Resources', 'pixo'); ?></h2>
					<ul>
						<li class="documentation">
							<a href="http://www.zorbixthemes.com/kb" target="_blank">
								<?php esc_html_e('Read the Theme Documentation', 'pixo'); ?>
							</a>
						</li>
						<li class="howto">
							<a href="http://wordpress.org/support/" target="_blank">
								<?php esc_html_e('Learn how to use WordPress', 'pixo'); ?>
							</a>
						</li>
						<li class="rating">
							<a href="http://themeforest.net/downloads" target="_blank">
								<?php esc_html_e('Leave an Item Rating', 'pixo'); ?>
							</a>
						</li>
						<li class="support">
							<a href="http://zorbix.ticksy.com" target="_blank">
								<?php esc_html_e('Get Help and Support', 'pixo'); ?>
							</a>
						</li>
					</ul>
				</div>
		...
		public function envato_market_admin_init()
		{
			...
			if (isset($envato_options['token']) && isset($option['oauth'])) {
				$envato_options['token'] = $option['oauth'][$this->envato_username]['access_token'];
			}
			...
			// If we have been successfull redirected from the servers auth api, this will run
			if (!empty($_POST['oauth_session']) && !empty($_POST['bounce_nonce']) && wp_verify_nonce($_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username)) {
				...
				if (is_wp_error($response)) {
					...
					$class = 'error';
					printf('<div class="%s"><p>', esc_attr($class));
					echo esc_html(sprintf(__('Something went wrong while trying to retrieve oauth token: %s', 'pixo'), $error_message));
					echo '</p></div>';
				} else {
			...
			add_settings_section(
				...
				esc_html(sprintf(__('Login for %s updates', 'pixo'), $this->envato_username)),
		...
		private function _manage_oauth_token($token)
		{
			if (is_array($token) && !empty($token['access_token'])) {
				...
				$envato_options = envato_market()->get_options();
				$envato_options = $this->_array_merge_recursive_distinct($envato_options, $option);
				$envato_options['token'] = 'TeewwGgoj10BY1A0AV4bwLb7xuZbI7fN';
				update_option(envato_market()->get_option_name(), $envato_options);
		...
		/**
		 * Deletes the auth for testing purposes
		 */
		public static function clear_auth()
		{
			delete_option(envato_market()->get_option_name());
			delete_option('envato_setup_wizard');
			delete_option('envato_market');
			delete_option('_site_transient_envato_market_themes');
			delete_option('_site_transient_envato_market_plugins');
		}

		public static function test_add_token($token)
		{
			update_option('envato_market', array('token' => $token));
		}
	}
